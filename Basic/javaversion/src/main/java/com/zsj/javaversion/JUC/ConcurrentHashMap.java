...
public class ConcurrentHashMap<K,V> extends AbstractMap<K,V>
     ...
     * 以保持在Java数组分配和索引内表示两个表大小的幂的界限
     */
    private static final int MAXIMUM_CAPACITY = 1 << 30;

    /**
     * 默认初始表容量。必须为2（即至少为1）的幂，并且最大为MAXIMUM_CAPACITY。
     */
    ...
    /**
     * 每个传输步骤的最小重新绑定数量。范围被细分以允许多个调整程序线程。
     *
     */
    ...
    public ConcurrentHashMap(int initialCapacity) {
        ...
        int cap = ((initialCapacity >= (MAXIMUM_CAPACITY >>> 1)) ?
                MAXIMUM_CAPACITY :
                //1.5 * initialCapacity + 1)，然后向上取最近的 2 的 n 次方
     ...
     *  最后在添加完成之后，会判断在该节点处共有多少个节点（注意是添加前的个数），如果达到8个以上了的话，并且大于64
    ...
    private final void addCount(long x, int check) {
        ...
        if ((as = counterCells) != null ||
            ...
            if (as == null || (m = as.length - 1) < 0 ||
                    (a = as[/*ThreadLocalRandom.getProbe()*/ 0& m]) == null ||
                    !(uncontended =
    ...
    private final void fullAddCount(long x, boolean wasUncontended) {
        ...
        for (;;) {
            ...
            if ((as = counterCells) != null && (n = as.length) > 0) {
                ...
                h = h;//ThreadLocalRandom.advanceProbe(h);
            }
